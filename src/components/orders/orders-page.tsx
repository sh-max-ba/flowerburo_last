  ...
  return (
    <div className={cn("flex flex-col gap-4 rounded-2xl border bg-white p-4", urgency.cardClass)}>
      ...
      <div className="flex flex-wrap justify-end gap-2">
        {order.status === "Новый" && (
          <Button size="sm" onClick={() => onStart(order)} disabled={pendingAction}>
            {pendingAction && <Spinner className="size-3.5" />}
            В работу
          </Button>
        )}
...
function ReadyConfirmButton({
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      <AlertDialogTrigger
        render={<Button size="sm" variant={primary ? "default" : "outline"} disabled={pending} />}
      >
