...
export function DealsKanban({
  ...
  // Once a refreshed board reflects an optimistic move (or the deal is gone), drop the stale
  // overlay. Render-time adjustment (keyed on board.deals identity) instead of an effect.
  const [lastBoardDeals, setLastBoardDeals] = useState(board.deals)
  if (lastBoardDeals !== board.deals) {
    setLastBoardDeals(board.deals)
...
function formatDateTime(value: string) {
  // Срок (due_at) — наивное локальное время, показываем ровно как введено (без сдвига пояса).
