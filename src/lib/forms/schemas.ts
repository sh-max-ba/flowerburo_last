...
// --- updateOrderFromDeal -----------------------------------------------------
// Правка уже созданного заказа из карточки сделки: нужны и сделка, и конкретный заказ.
// Состав (itemProductCode[]/itemQty[]/…) и deliveryPrice/courierPayout парсятся в транзакции.
export const DealOrderUpdateInputSchema = z.object({
  dealId: zNum().refine((value) => value !== 0, "Сделка не найдена."),
  orderId: zNum().refine((value) => value !== 0, "Заказ по сделке не найден."),
})

export type DealOrderUpdateInput = z.infer<typeof DealOrderUpdateInputSchema>
